...
class AES_GENERATE_FILES():
    ...
    def create_open_combined(self,path,mode):
        ...
        return files
    def __init__(self):
        print("\033[1;33mAES-128\tCBC MODE\tONLY COMBINED FILES\033[0m")
            ...
            with open(key_files, "rb") as key_file:
                while(True):
                    ...
                    for plaintext_files in [path_plaintext+f for f in os.listdir(path_plaintext) if os.path.isfile(os.path.join(path_plaintext,f))]:
                        ...
                        last_ciphertext = 0x0123456789ABCDEF0123456789ABCDEF
                        with open(plaintext_files, "rb") as plaintext_file:
                            ...
                            while(True):
                                ...
                                plaintext = int.from_bytes(plaintext, "big") ^ last_ciphertext
                                ciphertext = self.AES.encrypt(plaintext)
                                last_ciphertext = ciphertext[-1]
                                for i in range(0,10):
